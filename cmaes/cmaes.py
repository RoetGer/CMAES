    ...
    def minimize(self):
        ...
        for gen in range(1, self.maxgen + 1):
            y_weighted = np.sum(y[:self.elite_size] *
                             self.weights[:self.elite_size, np.newaxis], axis=0)
            upd_mean_vec = self.mean_vec + self.c_m*self.step_size*y_weighted
            ...
            if (max(diff_vec) < self.abs_tol or
                ...
                result_dict = {'best fvalue': best_fvalue,
                               ...
                               'abs/rel conv': (max(diff_vec),
                                                max(diff_vec/abs(self.mean_vec)))}
                return result_dict
            ...
            self.cov_mat = ((1 + self.c_1*(1 - h_sig)*self.c_c*(2 - self.c_c) -
                            self.c_1 - self.c_mu*sum(self.weights)) *
                            self.cov_mat + self.c_1*np.outer(self.p_c, self.p_c)
